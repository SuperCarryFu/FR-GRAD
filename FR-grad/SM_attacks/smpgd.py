...
class MetaPGD(ConstrainedMethod):
    def __init__(self, model,goal, distance_metric, eps=1.6,iters=20):
        super(MetaPGD, self).__init__(model,goal, distance_metric, eps)
        self.iters=iters
    ...
    def batch_grad_m(self,x, img_ft, alpha,model):
        ...
        for _ in range(6):
            ...
            x_neighbor = x_neighbor + torch.randn_like(x_neighbor)*sqrt(alpha)
            x_neighbor = torch.clamp(x_neighbor, min=0, max=255).detach().requires_grad_(True)
            ft = model.forward(x_neighbor)
            loss = self.getLoss(ft, img_ft)
            loss.backward()
            ...
            xx_neighbor = x_neighbor + torch.randn_like(x_neighbor)*sqrt(alpha)
            x_neighbor = torch.clamp(x_neighbor, min=0, max=255).detach().requires_grad_(True)
            ft = model.forward(x_neighbor)
            loss = self.getLoss(ft, img_ft)
            loss.backward()
    ...
    def attack(self, src,dict, models):
        tmp = src.clone().detach()
        tmp = tmp + torch.empty_like(tmp).uniform_(-self.eps, self.eps)
        tmp = torch.clamp(tmp, min=0, max=255).detach().requires_grad_(True)
        ...
        for _ in range(self.iters):
            ...
            for i in train_index:
                ...
                if self.distance_metric == 'linf':
                    ...
                    noise = (global_grad + 1.0 * grad) / (1.0 * (24 + 1.0))
                    # noise = (global_grad + 1.0 * grad) / (1.0 * (3 + 1.0))
